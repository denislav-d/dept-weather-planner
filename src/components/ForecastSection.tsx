            ...
            <div className="flex flex-col gap-y-6 pt-10 md:pt-24 col-span-full lg:col-span-5">
                ...
                <ul className="flex flex-col gap-y-2">
                    {forecast.forecast.map((day, index) => (
                        <li key={index} className="flex flex-col xl:flex-row xl:items-center xl:grid xl:grid-cols-3 xl:border-b xl:border-b-[#e5e5e5] xl:pb-3">
                            <h6 className="font-semibold md:col-span-1">{formatDate(day.date)}</h6>

                            <div className="grid grid-cols-5 md:col-span-2">
                                <span className="flex items-center gap-x-2 mr-6 col-span-2">
                                    <span aria-label={day.condition.description}>{formatWeatherIcon(day.condition.icon)}</span>

                                    <p className="font-medium!">{day.metric === "CELCIUS" ? day.minTemp : FahrenheitToCelsius(day.minTemp)}° / <span className="font-semibold">{day.metric === "CELCIUS" ? day.maxTemp : FahrenheitToCelsius(day.maxTemp)}°</span></p>
                                </span>
                                ...
                                <span className="flex items-center col-span-1 mr-2 md:mr-4 lg:mr-2">
